...
taxe %>% filter(time%in%c(2010, 2020)) %>% 
  ...
  labs(title= "Rata de taxare efectivă", 
       subtitle="Eurostat: earn_nt_taxrate",
       y="Rata de taxare efectivă", x="Ţară", fill="Anul", 
       caption="Figura x. Rata de taxare efectivă") +
...
df %>% filter(time%in%c(2014, 2019)) %>% 
  ...
  column_to_rownames("id") %>% kmeans(centers=4) %>% .[[1]] %>% 
  ...
  geom_point(aes(colour=time, shape=time), size=2)  +
  ...
  labs(title="Încasările din taxa pe profit vs venit", subtitle="% din PIB", 
       x="Venit", y="Profit", colour="An", shape="An", fill="Cluster") +
  ggrepel::geom_text_repel(aes(label=geo), max.overlaps = 7) +
  scale_fill_brewer(palette = "Set1") +
...
rate <- read_csv(here::here("database", "rate_praguri_venit_oecd_summary.csv"))

rate %>% filter(max_rate<100) %>% 
  ggplot(aes(x=time, y=max_rate, group=geo)) +
...
```{r, fig.height=8, fig.width=10, dpi=300}
rate %>% filter(time==2021) %>% 
  mutate(geo=fct_reorder(geo, nr_praguri)) %>%
  ggplot(aes(x=geo, y=nr_praguri)) +
  geom_col(fill='steelblue3') + 
  coord_flip() +
  labs(title="Număr de praguri de taxare", 
       x="An", y="Număr praguri") 
```
...
df %>% filter(!grepl("Croatia|Elvetia", geo)) %>% 
  ...
  geom_line() + 
  scale_y_continuous(labels = scales::percent_format(scale=1)) + 
  gghighlight::gghighlight(geo%in%c("Romania")) + 
  labs(title="Încasările din impozitul pe venit", 
       x="An", y="Procent din PIB", subtitle="Sursa: Eurostat gov_10a_taxag")
```
...
brms::as_draws_df(model) %>%  
  ...
  mutate(param = str_extract(string = param, pattern="(?<=\\[)[a-zA-Z\\s]+(?=,year)")) %>%
  ...
  ggridges::geom_density_ridges(fill='steelblue3', alpha=0.7, colour=NA) + 
  geom_vline(xintercept = 0, linetype=2, colour='red3', linewidth=1) +
  labs(title=latex2exp::TeX("$Valoarea\\,coeficienţilor\\,β_{1}^{ţară}$"), 
       x="% per an", y="Ţară", subtitle="Pantă", 
       caption ="Bayesian Latent Curve Growth Model")
```
...
brms::as_draws_df(model) %>%  
  ...
  labs(title=latex2exp::TeX("$Valoarea\\,coeficienţilor\\,β_{0}^{ţară}$"), 
       x="% per an", y="Ţară", subtitle="Intercept", 
       caption ="Bayesian Latent Curve Growth Model")
```
...
df %>% 
  ...
  ggplot(aes(x=time, y=income_revenues)) + geom_line(colour='red3') + 
...
plot.CausalImpact <- function(model){
  ...
  g1 <- x %>% ggplot(aes(x=time, y=response)) + 
    geom_line(size=1) + 
    geom_line(aes(x=time, y=point.pred), colour='red3', linetype=2) +
    geom_vline(xintercept = x$time[model$model$pre.period[2]] + 1, linetype=2, colour='grey50') +
    geom_ribbon(aes(x=time, ymin=point.pred.lower, ymax=point.pred.upper), 
                alpha=0.2, fill='red3') +
    labs(caption="Seria originală plus contrafactualul", x=NULL)
  
  g2 <- x %>% ggplot(aes(x=time, y=point.effect)) + 
    geom_line(linetype=2, colour='red3') + 
    geom_ribbon(aes(x=time, ymin=point.effect.lower, 
                    ymax=point.effect.upper), alpha=0.3, fill='red3') +
    geom_vline(xintercept=x$time[model$model$pre.period[2]] + 1, linetype=2, colour='grey50') +
    geom_hline(yintercept=0, linetype=2) +
    labs(caption="Impactul renunţării la cota unică", x=NULL)
  
  g3 <- x %>% ggplot(aes(x=time, y=cum.effect)) + 
    geom_line(linetype=2, colour='red3') + 
    geom_ribbon(aes(x=time, ymin=cum.effect.lower, ymax=cum.effect.upper), 
                alpha=0.3, fill='red3') +
    geom_vline(xintercept=x$time[model$model$pre.period[2]]+ 1, linetype=2, colour='grey50') +
    geom_hline(yintercept=0, linetype=2) +
    labs(caption="Impactul cumulativ", x=NULL)
  g1 / g2 / g3
}
...
```{r, fig.width=10, fig.height=8, dpi=300}
corporate_rate <- read_csv(here::here("database", "income_profit_gdp_inflation.csv"))

ggplot(corporate_rate,aes(x=time, y=profit_rate, group=geo)) + 
  geom_line() + gghighlight::gghighlight(geo=="Romania") +
  scale_y_continuous(labels=scales::percent_format(scale=1)) +
  labs(title="Rata impozării pe profitul companiilor", x="An", 
       y="Procent din profit", subtitle="Sursa: TaxFoundation")
...
culori <- c("2010"="red3", "2020"="steelblue3")
...
corporate_rate %>% filter(time%in%c(2010, 2020)) |> 
  ...
  geom_point(aes(colour="2010"), size=1) + 
  geom_point(aes(x=geo, y=`2020`, colour="2020"), size=1) + 
  coord_flip() + labs(title="Impozitul pe profit", 
                      subtitle="Sursa: TaxFoundation", 
...
```{r}
distrib <- read_csv(here::here("database", "ILO_Romania.csv")) %>% 
           filter(time==2019) %>% select(-time)


raport <- function(coefs, k){
((coefs[1])*sum(distrib$obs_value[1:k]) + 
    (coefs[2])*sum(distrib$obs_value[(k+1):10]))/(0.1*sum(distrib$obs_value[1:k]) + 
                                              0.1*sum(distrib$obs_value[(k+1):10]))
}
...
```{r}
# aici calculăm percentila în care se încadrează un venit de 2200-2300 de EURO. Este #percentila 92 % la venitul naţional pre-taxe.

read_delim(here::here("database", "WID", "WID_data_RO.csv"), delim=";") %>% 
  filter(variable=="adiincj992") %>% 
  filter(between(value, 2200*12*5, 2300*12*5)) %>% 
  filter(year==2019) %>% pull(percentile)
```
...
multiplicator <- expand_grid("cota_1"=seq(0, 0.5, by=0.01), 
  ...
  mutate(ratio = map_dbl(coefs, ~raport(.x, k=9))) %>% 
  mutate(ratio2 = cut(ratio, breaks=c(-0.01, 1, 2, 3, 4, 5))) %>% 
...
Însă nu propunem şi cotele Slovaciei, ci ceva mai modest: 16 % ( în loc de 19 %) şi 21 % (în loc de 25 %). Impozitul pe venitul din salarii a fost de circa 28 de miliarde de RON în 2021 conform [MF](https://mfinante.gov.ro/documents/35673/162114/nota_bgc31dec2021.pdf). Asumând impozitul pe venit slovac pe veniturile din salarii s-ar cumula circa 20 de miliarde de RON la buget. Pentru anul 2021. Nu mai ajustăm în jos această sumă deoarece colectările vor fi cu siguranţă semnificativ mai mari în 2023.

### Calcul alternativ folosind datele WID
...
Obţinem un surplus de 78.6 % foarte similar cu cel calculat pe datele de la ILOSTAT, care dă doar 71.8 %. Vom rămâne însă pe scenariul calculat pe datele ILOSTAT pentru că ne oferă o concluzie mai conservatoare.
